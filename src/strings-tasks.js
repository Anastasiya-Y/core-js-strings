...
function removeFirstOccurrences(str, value) {
  ...
  const newStr = str.slice(strIndex, strIndex + value.length);
  const finalStr = str.replace(newStr, '');
  return finalStr;
}
...
function sumOfCodes(str) {
  ...
  for (let i = 0; i < testedStr.length; i += 1) {
    const charIndex = testedStr.charCodeAt(i);
    sum += charIndex;
  }
  return sum;
}
...
function isPalindrome(str) {
  const string = str.toLowerCase().replaceAll(/[^\w]/g, '');
...
function encodeToRot13(str) {
  ...
  const rot13 = Array.from(
    'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'
  );

  const newStr = str.replace(regExp, (char) => {
    const index = alphabet.indexOf(char);
    const newChar = rot13[index];
    return newChar;
  });
  return newStr;
}
